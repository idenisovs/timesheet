...
The KEY and CRT files are actually used by Angular Development server, if you run it like `ng serve --ssl` (so dev version of Timesheet will be accessible via https://localhost:4200/).
...
If you want browsers to trust the SSL certificates, you have to add the `dev-ca.crt` file to `/usr/local/share/ca-certificate` (Ubuntu) or Keychain Access (macOS).
